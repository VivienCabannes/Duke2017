...
class SparseClassifier:
    def __init__(self, gamma_sel = .1, gamma_up = 0.01, sel_func = lasso,
...
class FeatureElimination:
    ...
    def fit(self, design, labels):
        ...
        for k in range(len(self.I)-self.nb):
            X = design[:,self.I]
            self.met.fit(X, labels)
            i = np.argmin(np.abs(self.met.w))
            index = tmp[self.I][i]
            self.I[index] = False
        X = design[:, self.I]
        self.met.fit(X, labels)
...
def split(design, labels, thres=.2, nb_points=8, lin_method = met):
...
def build_tree(design, labels, ctl = True, split_func = split, 
               thres = .2, nb_points = 8, lin_method = met):
    test_or_diag, design_splits = split_func(design, labels, thres = thres, 
                                             nb_points=nb_points, 
    ...
    if len(design_splits)==1:
        ...
        for design_split in design_splits:
            ...
            son_test = build_tree(d, l, ctl = False, split_func = split_func,
...
class DecisionTree:
    def __init__(self, split_func = split, thres = .2, nb_points = 8, 
                 lin_method = met):  
    ...
    def fit(self, design, labels):
        self.tree = build_tree(design, labels, split_func = self.split_func,
